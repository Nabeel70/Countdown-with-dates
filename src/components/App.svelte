...
<div class="container" class:alt={alt}>
  <p id="spcl">ARE YOU READY? <span>{`${date.getDate()} ${date.toLocaleString('default', { month: 'short' })} ${date.getFullYear()}`}</span></p>
...
<style>
  ...
  .alt .cell {
      background-color:rgba(233, 237, 251);
  }
  .cell:not(:last-child) {
